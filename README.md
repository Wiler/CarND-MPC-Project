...
# Handling 100 ms latency

Just before using the solve function of the MPC class, an additional calculation is done. There all values (px, py, v, psi, cte, epsi) are recalculated using the kinematic model with delta = 0.1 s and Lf = 2.67, in order to feed the MPC with correct values of the variables 100 ms after the actual time.
